...
def metric_card(title: str):
    ...
    card = ui.card().classes(
        'bg-black/30 text-white min-w-[160px] cursor-pointer '
...
def build_trend_options() -> Dict[str, Any]:
    ...
    return {
        ...
        'xAxis': {'type': 'category', 'data': [], 'axisLine': {'lineStyle': {'color': '#888'}}, 'axisLabel': {'color': '#bbb'}},
        'yAxis': y_axes,
        'series': series,
    }
...
with ui.row().classes('items-center justify-between w-full'):
    with ui.row().classes('items-center gap-4'):
        ...
        inp_hours = ui.number(label='Hours', value=0.5, min=0.05, max=24, step=0.05).classes('w-[140px]')
        sel_bucket = ui.select(
            {
                1: '1 s', 2: '2 s', 5: '5 s', 10: '10 s',
                15: '15 s', 30: '30 s', 60: '60 s',
            },
            value=5,
            label='Bucket'
        ).classes('w-[140px]')
    ...
    with ui.row().classes('items-center gap-3'):
        ...
        air_set_slider = ui.slider(min=160, max=210, step=0.5, value=190).props('label color=teal').classes('w-[300px]')
        ...
        ui.button('적용', on_click=lambda: asyncio.create_task(apply_air_setpoint(float(air_set_slider.value)))).classes('ml-1')

    # 명령 슬라이더 (Pump Hz)
    with ui.row().classes('items-center gap-3'):
        ui.label('Pump Hz Setpoint (Hz)').classes('text-white/70')
        pump_hz_slider = ui.slider(min=20, max=60, step=0.5, value=45).props('label color=teal').classes('w-[260px]')
        async def apply_pump_hz(v: float) -> None:
            try:
                async with httpx.AsyncClient(timeout=5.0) as client:
                    await client.post(f'{API_BASE}/api/v1/commands/pump_hz_setpoint', json={'value': v})
                ui.notify(f'Pump Hz applied: {v:.1f} Hz', color='positive')
            except Exception as e:
                ui.notify(f'Pump Hz failed: {e}', color='negative')
        ui.button('적용', on_click=lambda: asyncio.create_task(apply_pump_hz(float(pump_hz_slider.value)))).classes('ml-1')
...
# 단일 지표 확대 다이얼로그
single_metric_key: Optional[str] = None
with ui.dialog() as single_dialog, ui.card().classes('w-[1000px] max-w-[95vw] bg-black/30'):
    ...
    trend_single: Any = ui.echart({
        ...
        'xAxis': {'type': 'category', 'data': [], 'axisLine': {'lineStyle': {'color': '#888'}}, 'axisLabel': {'color': '#bbb'}},
        'yAxis': {'type': 'value', 'name': '', 'axisLine': {'lineStyle': {'color': '#888'}}, 'axisLabel': {'color': '#bbb'}},
...
with ui.dialog() as trend_dialog, ui.card().classes('w-[1100px] max-w-[95vw] bg-black/30'):
    ui.label('Realtime Trend (DO, Air (L/min), MLSS, Temp, pH, Power)').classes('text-white text-lg mb-2')
                ...
                if r2.status_code == 200:
                    ...
                    if single_metric_key:
                        ...
                        if meta:
                            ...
                            trend_single.options['series'][0]['data'] = [row.get(meta['key']) for row in rows]
