...
class SqlAlchemyHistorian(HistorianPort):
    ...
    async def write(self, sample: Telemetry) -> None:
        row = TelemetryRow(
            ts=sample.ts, DO=sample.DO, MLSS=sample.MLSS, temp=sample.temp,
            pH=sample.pH, air_flow=sample.air_flow, power=sample.power,
    ...
    async def write_many(self, samples: Sequence[Telemetry]) -> None:
        rows = [
            TelemetryRow(
                ts=s.ts, DO=s.DO, MLSS=s.MLSS, temp=s.temp, pH=s.pH,
                air_flow=s.air_flow, power=s.power, total_energy_calc=s.total_energy_calc,
            )
            for s in samples
        ]
    ...
    async def query(self, since: datetime) -> List[Telemetry]:
        ...
        return [
            Telemetry(
                ts=r.ts, DO=r.DO, MLSS=r.MLSS, temp=r.temp, pH=r.pH,
                air_flow=r.air_flow, power=r.power, total_energy_calc=r.total_energy_calc,
            )
            for r in rows
        ]
