...
class CommandRepo:
    async def insert(
        ...
        *,
        ...
        row = Command(
            id=id, ts=ts, unit_id=unit_id, kind=kind, value=value,
            state=state, dry_run=dry_run, priority=priority, idempotency_key=idempotency_key,
        )
        session.add(row)
        await session.flush()
    ...
    async def get_by_idempotency_key(self, session: AsyncSession, key: str) -> Optional[Command]:
        stmt = select(Command).where(Command.idempotency_key == key)
        res = await session.execute(stmt)
        rows: ScalarResult[Command] = res.scalars()
        return rows.first()

    async def get_by_id(self, session: AsyncSession, id: str) -> Optional[Command]:
    ...
    async def list_queued(self, session: AsyncSession, limit: int = 50) -> list[Command]:
        stmt = select(Command).where(Command.state == "queued").order_by(Command.ts.asc()).limit(limit)
        res = await session.execute(stmt)
        rows: ScalarResult[Command] = res.scalars()
        return list(rows.all())
